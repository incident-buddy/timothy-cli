...
LLMが生成したHTMLをターミナルからアップロードし、署名付きURLで共有するセルフホスト型CLIツールです。URLを知っている人だけが閲覧でき、閲覧者のログインは不要です。
...
URLはCloud Run上のAPIを経由して配信され、IPアドレス制限で保護されます。ストレージはプライベート設定で、ファイルに直接アクセスすることはできません。
...
| Cloud Storage | パブリックアクセス禁止・署名付きURLのみで配信 |
